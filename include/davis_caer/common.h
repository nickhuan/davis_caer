...
static inline void *caerGenericEventGetEvent(void *headerPtr, uint32_t n) {
	...
	if (n >= caerEventPacketHeaderGetEventCapacity((caer_event_packet_header*)headerPtr)) {
#if !defined(LOG_NONE)
		...
		caerLog(LOG_ERROR, "caerGenericEventGetEvent()",
			...
			n, caerEventPacketHeaderGetEventCapacity((caer_event_packet_header*)headerPtr));
#endif
		return (NULL);
	}
	...
	return (((uint8_t *) headerPtr)
		+ (sizeof(struct caer_event_packet_header) + (n * caerEventPacketHeaderGetEventSize((caer_event_packet_header*)headerPtr))));
}

static inline uint32_t caerGenericEventGetTimestamp(void *eventPtr, void *headerPtr) {
	return (le32toh(*((uint32_t *) (((uint8_t *) eventPtr) + caerEventPacketHeaderGetEventTSOffset((caer_event_packet_header*)headerPtr)))));
}
